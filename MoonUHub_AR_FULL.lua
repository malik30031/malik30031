...
local معلوماتTab = Window:MakeTab({ Title = "معلومات", Icon = "rbxassetid://15309138473" })



معلوماتTab:AddSection({ "معلوماتrmações do Script" })
معلوماتTab:AddParagraph({ "المالك / المطور:", "Herry And e0davizinTA." })
معلوماتTab:AddParagraph({ "المساهمين:", "Blue, sukuna, Magekko, Darkness, Star, Toddy" })
معلوماتTab:AddParagraph({ "أنت تستخدم:", "MoonU Hub Brookhaven " })
معلوماتTab:AddParagraph({"المشغل الخاص بك:", executor})

معلوماتTab:AddSection({ "إعادة الانضمام" })
معلوماتTab:AddButton({
    Name = "إعادة الانضمام",
    Callback = function()
...
local TrollTab = Window:MakeTab({ Title = "سكربتات الترفيه", Icon = "rbxassetid://13364900349" })


TrollTab:AddSection({ "الثقب الأسود" })
TrollTab:AddButton({
    Name = "الثقب الأسود",
...
Label.Text = "إحضار القطع | Made by: Lusquinha_067"
...
TrollTab:AddSection({ "أفاتار RGB" })
...
-- Tab 4: منع الجلوس
TrollTab:AddSection({ "منع الجلوس" })
TrollTab:AddToggle({
    Name = "منع الجلوس",
    Description = "يمنع شخصيتك من الجلوس",
    Default = false,
    Callback = function(Value)
...
local Troll = Window:MakeTab({ Title = "التحكم باللاعبين", Icon = "rbxassetid://131153193945220" })
...
local function KillWithCouch()
    ...
    local PlayerSection = Troll:AddSection({ Name = "التحكم باللاعب" })
    ...
    local MethodSection = Troll:AddSection({ Name = "طرق" })

    Troll:AddDropdown({
        Name = "اختر طريقة القتل",
...
local NOME_FERRAMENTA = "Click كرة الدفع"
local ferramentaEquipada = false
...
Tab:AddParagraph({"معلومات", "Loop de tocar Áudio (Todos players do Server ouvem)"})
...
Tab:AddParagraph({"معلومات", "Todos do server ouvem o áudio"})
...
CarTab:AddParagraph({"معلوماتrmação:", "Recomendo usar 2 vezes para garantir que todos os veículos sejam removidos"})
...
CarTab:AddParagraph({"معلوماتrmação:", "Puxa todos os carros do servidor até você"})
...
CarTab:AddParagraph({"معلوماتrmação:", "Ambos os turbos não precisam de Gamepass"})
...
CarTab:AddParagraph({"معلوماتrmação:", "Spamar vários carros"})
